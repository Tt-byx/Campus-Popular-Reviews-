...
class JwtUtilsTest {
    ...
    void testCreateJwt_withUserRole() {
        String token = jwtUtils.createJwt(1L, "testuser", "user");
        assertNotNull(token);
    ...
    void testCreateJwt_withAdminRole() {
        String token = jwtUtils.createJwt(2L, "admin", "admin");
        assertNotNull(token);
    }
    ...
    void testCreateJwt_withSuperAdminRole() {
        String token = jwtUtils.createJwt(3L, "superadmin", "super_admin");
        assertNotNull(token);
    }
    ...
    void testCreateJwt_withNullRole() {
        String token = jwtUtils.createJwt(1L, "testuser", null);
        assertNotNull(token);
    ...
    void testDifferentUsersDifferentTokens() {
        String token1 = jwtUtils.createJwt(4L, "user1", "user");
        String token2 = jwtUtils.createJwt(5L, "user2", "user");
    ...
    void testSameUserTokensDiffer() throws Exception {
        String token1 = jwtUtils.createJwt(1L, "testuser", "user");
        Thread.sleep(1000);
        String token2 = jwtUtils.createJwt(1L, "testuser", "user");
        assertNotEquals(token1, token2);
    }
    ...
    void testRoleAccuracy() {
        String userToken = jwtUtils.createJwt(6L, "u1", "user");
        String adminToken = jwtUtils.createJwt(7L, "a1", "admin");
        String superAdminToken = jwtUtils.createJwt(8L, "s1", "super_admin");
    ...
    void testUsernameAccuracy() {
        String token = jwtUtils.createJwt(9L, "specific-user-name", "user");
    ...
    void testSubjectField() {
        String token = jwtUtils.createJwt(10L, "myuser", "admin");
        Claims claims = jwtUtils.parseJwt(token);
    ...
    void testCreateJwt_withEmptyUsername() {
        String token = jwtUtils.createJwt(11L, "", "user");
        assertNotNull(token);
    ...
    void testCreateJwt_withLongUsername() {
        ...
        String token = jwtUtils.createJwt(12L, longName, "user");
        assertNotNull(token);
